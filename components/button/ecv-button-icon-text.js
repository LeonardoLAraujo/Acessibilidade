...
import { ECVButtonIcon } from './ecv-button-icon.js';
 ...
 * @class
 */
export class ECVButtonIconText extends ECVButtonIcon{
    ...
    render(){
        ...
        align-items: ${this.alignItems}; font-size: ${this.fontSize}; cursor: ${this.cursor}; box-shadow: ${this.boxShadow};`;

        return html`
            <slot style=${myStyles}>
            </slot>
            <button @click=${this.function} style=${myStyles} name=${this.name} paddingAll=${this.buttonPadding} @mouseover=${this.mouseOver} @mouseleave=${this.mouseLeave}>
                <ecv-icon name=${this.nameIcon} fontSize=${this.iconSize} color=${this.colorIcon} paddingAll="0px 2px" ></ecv-icon>
                <ecv-paragraph text=${this.text} fontFamily=${this.fontFamily} color=${this.colorParagraph} colorHover=${this.colorHover}></ecv-paragraph>
            </button>
 ...
 customElements.define('ecv-button-icon-text', ECVButtonIconText);